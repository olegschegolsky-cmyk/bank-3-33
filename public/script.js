...
let cart = []; let ws = null; let html5QrCode = null; let confirmedActionCallback = null;
let currentEditUserId = null; let currentEditShopItemId = null;
let chartInstance = null; let currentExchangeTab = 'crypto'; let currentAssetId = null; let currentChartAssetId = null;
  ...
  ws.onmessage = (event) => {
    ...
    switch (message.type) {
        ...
        case 'exchange_update_required':
            ...
            break;
        case 'news_update_required':
            if (document.getElementById('newsModal')?.style.display === 'flex') loadNews();
            break;
...
async function login() {
  try {
    const response = await fetch(`${API_BASE_URL}/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ login: document.getElementById('username').value.trim(), password: document.getElementById('password').value }) });
    const result = await response.json();
    if (response.ok && result.success) {
      localStorage.setItem('authToken', result.token); localStorage.setItem('isAdmin', result.isAdmin);
      if (result.isAdmin) { window.location.href = 'admin.html'; return; }
      await loadInitialData();
      document.getElementById('login').style.display = 'none'; document.getElementById('app-content').style.display = 'block'; document.getElementById('menu').style.display = 'flex'; document.getElementById('bottom-bar').style.display = 'flex'; initializeWebSocket();
...
async function adminLogin() {
    try {
        ...
        if (response.ok && result.success && result.isAdmin) {
            localStorage.setItem('authToken', result.token); localStorage.setItem('isAdmin', true);
...
function updateAllDisplays(){
  ...
  updateTransactionHistoryDisplay(); updateCartModalItemCount();
}
...
function updateTransactionHistoryDisplay() {
  ...
  const grouped = appData.transactions.reduce((acc, t) => { const txDate = new Date(t.timestamp.replace(' ', 'T') + 'Z'); const dateKey = txDate.toLocaleDateString('uk-UA', { day: 'numeric', month: 'long', year: 'numeric' }); if (!acc[dateKey]) acc[dateKey] = []; acc[dateKey].push(t); return acc; }, {});
  listDiv.innerHTML = Object.keys(grouped).map(dateKey => `<div class="transaction-date-group">${dateKey}</div>${grouped[dateKey].map(t => { const isPositive = t.amount > 0; const txDate = new Date(t.timestamp.replace(' ', 'T') + 'Z'); return `<div class="transaction-item"><div class="transaction-icon">${getTransactionIconByType(t.type)}</div><div class="transaction-info"><span class="transaction-action">${getTransactionTitle(t)}</span><span class="transaction-comment">${t.comment}</span><span class="transaction-time">${txDate.toLocaleTimeString('uk-UA')}</span></div><span class="transaction-amount ${isPositive ? 'positive' : 'negative'}">${isPositive ? '+' : ''}${parseFloat(t.amount).toFixed(2)}</span></div>`}).join('')}`).join('');
}

function getTransactionIconByType(type) { if (type.includes('deposit')) return '🏦'; if (type.includes('transfer')) return '💸'; if (type.includes('purchase')) return '🛍️'; if (type.includes('exchange')) return '📈'; if (type.includes('task_reward')) return '🎁'; if (type.includes('admin')) return '⚙️'; return '💳'; }
function getTransactionTitle(t) { if (t.type === 'transfer') return t.amount > 0 ? `Отримано від ${t.counterparty}` : `Переказ до ${t.counterparty}`; if (t.type === 'purchase') return 'Покупка в магазині'; if (t.type === 'admin_adjustment') return t.amount > 0 ? 'Поповнення' : 'Зняття'; if (t.type === 'deposit') return 'Розміщення на депозит'; if (t.type === 'deposit_payout') return 'Виплата з депозиту'; if (t.type === 'exchange_buy') return 'Купівля активу'; if (t.type === 'exchange_sell') return 'Продаж активу'; if (t.type === 'task_reward') return 'Виконано завдання'; return 'Операція'; }
...
async function loadNews() {
    ...
    if (r.ok) {
        ...
        if (news.length === 0) { list.innerHTML = '<p class="no-transactions">Новин поки немає.</p>'; return; }
        list.innerHTML = news.map(n => {
            const dt = new Date(n.timestamp.replace(' ', 'T') + 'Z').toLocaleString('uk-UA');
            return `<div class="task-card" style="border-left: 4px solid var(--primary-color);">
                <div class="task-header"><h4 class="task-title">${n.title}</h4></div>
                <p class="task-desc" style="margin-top:0.5rem;">${n.content}</p>
                <div class="task-status" style="color:var(--text-disabled);">${dt}</div>
            </div>`;
        }).join('');
    }
}
...
function renderTasks() {
    ...
    AUTO_TASKS.forEach(task => {
        ...
        html += `<div class="task-card ${isCompleted ? 'completed' : ''}"><div class="task-header"><h4 class="task-title">${task.title}</h4><span class="task-reward">+${task.reward} грн</span></div><p class="task-desc">${task.desc}</p><div class="task-status ${isCompleted ? 'done' : 'todo'}">${isCompleted ? '✅ Отримано' : '⏳ Виконайте в додатку'}</div></div>`;
    });
    if(appData.adminTasks && appData.adminTasks.length > 0) {
        html += `<h4 style="margin-top:1.5rem; margin-bottom:0.5rem; text-align:center;">Ексклюзивні завдання</h4>`;
        appData.adminTasks.forEach(task => {
            const isCompleted = appData.completedTasks.includes(`admin_${task.id}`);
            html += `<div class="task-card ${isCompleted ? 'completed' : ''}" style="border-left: 4px solid var(--primary-color);"><div class="task-header"><h4 class="task-title">${task.title}</h4><span class="task-reward">+${task.reward} грн</span></div><p class="task-desc">${task.description}</p><div class="task-status ${isCompleted ? 'done' : 'todo'}">${isCompleted ? '✅ Отримано' : '📝 Надішліть підтвердження адміну'}</div></div>`;
        });
    }
    list.innerHTML = html;
}
...
async function confirmSendMoney() { const amount = parseFloat(document.getElementById('sendAmount').value); const recipientFullName = document.getElementById('sendTo').value.trim(); if (isNaN(amount) || amount <= 0) return alert('Введіть коректну суму.'); if (!recipientFullName) return alert('Введіть ПІБ отримувача.'); document.getElementById('confirmMessage').textContent = `Надіслати ${amount.toFixed(2)} грн до ${recipientFullName}?`; confirmedActionCallback = async () => { try { const response = await fetchWithAuth('/api/transfer', { method: 'POST', body: JSON.stringify({ recipientFullName, amount, comment: 'Приватний переказ' }), }); const result = await response.json(); alert(result.message); if(response.ok) closeModal('sendMoneyModal'); } catch (e) { alert('Помилка переказу.'); } }; openModal('confirmModal'); }
...
function confirmDeposit() { const amount = parseFloat(document.getElementById('depositAmount').value); const days = parseInt(document.getElementById('depositDays').value); if (isNaN(amount) || amount <= 0) return alert('Введіть коректну суму.'); document.getElementById('confirmMessage').textContent = `Відкрити депозит на ${amount.toFixed(2)} грн терміном на ${days} дн.?`; confirmedActionCallback = async () => { try { const response = await fetchWithAuth('/api/deposits', { method: 'POST', body: JSON.stringify({ amount, days }) }); const result = await response.json(); alert(result.message); if(response.ok) closeModal('confirmModal'); } catch(e) { alert('Помилка відкриття депозиту.'); } }; openModal('confirmModal'); }
function renderDeposits() { const list = document.getElementById('userDepositsList'); if (!list) return; if(!appData.deposits || appData.deposits.length === 0) { list.innerHTML = '<p class="no-transactions" style="padding:1rem;">У вас ще немає депозитів.</p>'; return; } const now = new Date(); list.innerHTML = appData.deposits.map(d => { const endTime = new Date(d.end_time.replace(' ', 'T') + 'Z'); const isMature = now >= endTime; let statusHtml = ''; if(d.status === 'completed') statusHtml = `<span style="color:var(--accent-color); font-weight:600;">Завершено</span>`; else if(d.status === 'cancelled') statusHtml = `<span style="color:var(--danger-color); font-weight:600;">Скасовано</span>`; else if(isMature) statusHtml = `<button class="action-button primary-button" style="padding:0.6rem; width:100%; margin-top:0.5rem;" onclick="claimDeposit(${d.id})">Забрати ${d.expected_payout.toFixed(2)} грн</button>`; else statusHtml = `<span style="color:var(--warning-color); font-weight:600;">До ${endTime.toLocaleString('uk-UA')}</span>`; return `<div class="event-item" style="border-left-color: ${d.status === 'active' ? 'var(--warning-color)' : d.status === 'completed' ? 'var(--accent-color)' : 'var(--text-disabled)'};"><h4 style="margin-bottom:0.25rem;">Сума: ${d.amount.toFixed(2)} грн</h4><p>Очікувана виплата: <strong>${d.expected_payout.toFixed(2)} грн</strong></p><div style="margin-top:0.5rem">${statusHtml}</div></div>`; }).join(''); }
...
async function loadExchangeData() { try { const response = await fetchWithAuth('/api/exchange/data'); if (response.ok) { appData.exchange = await response.json(); renderExchangeTab(); } } catch(e) {} }
...
function renderExchangeTab() { if (!appData.exchange) return; const listContainer = document.getElementById('exchangeAssetList'); const assets = appData.exchange.assets.filter(a => a.type === currentExchangeTab); if (assets.length === 0) { listContainer.innerHTML = '<p class="no-transactions">Активи відсутні.</p>'; return; } listContainer.innerHTML = assets.map(a => `<div class="asset-card ${currentAssetId === a.id ? 'active' : ''}" onclick="selectAsset(${a.id})"><span class="asset-card-symbol">${a.symbol}</span><span class="asset-card-name">${a.name}</span><span class="asset-card-price">${a.price.toFixed(2)} грн</span></div>`).join(''); if (currentAssetId) { selectAsset(currentAssetId); } else { selectAsset(assets[0].id); } }
function selectAsset(id) { currentAssetId = id; const asset = appData.exchange.assets.find(a => a.id === id); if(!asset) return; document.querySelectorAll('.asset-card').forEach(c => c.classList.remove('active')); const cards = document.querySelectorAll('.asset-card'); const index = appData.exchange.assets.filter(a => a.type === currentExchangeTab).findIndex(a => a.id === id); if(cards[index]) cards[index].classList.add('active'); document.getElementById('tradingZone').style.display = 'block'; document.getElementById('currentAssetName').textContent = `${asset.name} (${asset.symbol})`; document.getElementById('currentAssetPrice').textContent = `${asset.price.toFixed(2)} грн`; const portItem = appData.exchange.portfolio.find(p => p.asset_id === id); document.getElementById('currentAssetOwned').textContent = portItem ? portItem.amount.toFixed(4) : '0'; drawChart(id, asset.name); }

function drawChart(assetId, assetName) {
    const ctx = document.getElementById('exchangeChart').getContext('2d');
    const historyData = appData.exchange.history[assetId] || [];
    const labels = historyData.map(h => { const d = new Date(h.time.replace(' ', 'T') + 'Z'); return `${d.getHours()}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')}`; });
    const dataPoints = historyData.map(h => h.price);

    const isDown = dataPoints.length > 1 && dataPoints[dataPoints.length - 1] < dataPoints[0];
    const color = isDown ? '#ef4444' : '#10b981';
    const bgColor = isDown ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)';
    ...
    chartInstance = new Chart(ctx, {
        type: 'line',
        data: { labels: labels, datasets: [{ label: assetName, data: dataPoints, borderColor: color, backgroundColor: bgColor, borderWidth: 2, fill: true, tension: 0.3, pointRadius: 2, pointHoverRadius: 5 }] },
        options: { responsive: true, maintainAspectRatio: false, animation: { duration: 800, easing: 'easeOutQuart' }, scales: { x: { display: false }, y: { ticks: { color: '#9ca3af', font: { family: 'Inter' } }, grid: { color: 'rgba(255,255,255,0.05)' } } }, plugins: { legend: { display: false }, tooltip: { backgroundColor: 'rgba(31, 41, 55, 0.9)', titleColor: '#fff', bodyColor: color, bodyFont: { weight: 'bold' }, callbacks: { label: (ctx) => `${ctx.raw.toFixed(2)} грн` } } } }
    });
}

function executeTrade(type) { const amount = parseFloat(document.getElementById('tradeAmount').value); if (isNaN(amount) || amount <= 0) return alert('Введіть коректну кількість.'); const asset = appData.exchange.assets.find(a => a.id === currentAssetId); const actionName = type === 'buy' ? 'Купити' : 'Продати'; const totalCost = asset.price * amount; document.getElementById('confirmMessage').textContent = `${actionName} ${amount} ${asset.symbol} за ~${totalCost.toFixed(2)} грн?`; confirmedActionCallback = async () => { try { const response = await fetchWithAuth(`/api/exchange/${type}`, { method: 'POST', body: JSON.stringify({ assetId: currentAssetId, amount }) }); const result = await response.json(); alert(result.message); if(response.ok) { document.getElementById('tradeAmount').value = ''; closeModal('confirmModal'); } } catch(e) { alert('Помилка торгів.'); } }; openModal('confirmModal'); }

function showShop() { populateShopItems(); openModal('shopModal'); updateActiveNavButton('shop'); }
function showLeaderboard() { const list = document.getElementById('leaderboardList'); if (!appData.leaderboard || appData.leaderboard.length === 0) list.innerHTML = '<p class="no-transactions">Рейтинг порожній.</p>'; else list.innerHTML = appData.leaderboard.map((u, index) => `<div class="event-item" style="border-left-color: ${index === 0 ? '#fbbf24' : index === 1 ? '#94a3b8' : index === 2 ? '#b45309' : 'var(--primary-color)'};"><h4><span style="font-size: 1.2rem; margin-right: 0.5rem;">${index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}.`}</span> ${u.full_name}</h4><p style="margin-top: 0.5rem;"><strong>Баланс:</strong> ${parseFloat(u.balance).toFixed(2)} грн</p>${u.team_name ? `<p><strong>Команда:</strong> ${u.team_name}</p>` : ''}</div>`).join(''); openModal('leaderboardModal'); }
function populateShopItems(sortBy = 'default') { const shopGrid = document.getElementById('shopItems'); let items = [...appData.shopItems]; items.sort((a, b) => { if (sortBy === 'price-low') return (a.discount_price || a.price) - (b.discount_price || b.price); if (sortBy === 'price-high') return (b.discount_price || b.price) - (a.discount_price || a.price); return b.popularity - a.popularity; }); shopGrid.innerHTML = items.length ? items.map(item => { const hasDiscount = item.discount_price && item.discount_price < item.price; const price = hasDiscount ? item.discount_price : item.price; return `<div class="shop-item-card" onclick="addItemToCart(${item.id}, 1)"><img src="${item.image || './logo.png'}" alt="${item.name}" class="shop-item-image"><h4 class="shop-item-name">${item.name}</h4><div class="shop-item-price-container">${hasDiscount ? `<span class="shop-item-price-original">${item.price.toFixed(2)} грн</span>` : ''}<span class="shop-item-price">${price.toFixed(2)} грн</span></div><button class="action-button add-to-cart-button">Додати</button></div>`; }).join('') : '<p class="no-transactions">Товарів ще немає.</p>'; }
...
function showEventHistoryModal() { const list = document.getElementById('eventHistoryList'); if (!appData.transactions || appData.transactions.length === 0) list.innerHTML = '<p class="no-transactions">Історія порожня.</p>'; else list.innerHTML = appData.transactions.map(t => { const txDate = new Date(t.timestamp.replace(' ', 'T') + 'Z'); return `<div class="event-item ${t.type}"><h4>${getTransactionTitle(t)}</h4><p><strong>Сума:</strong> ${parseFloat(t.amount).toFixed(2)} грн</p><p><strong>Дата:</strong> ${txDate.toLocaleString('uk-UA')}</p></div>`}).join(''); openModal('eventHistoryModal'); }
...
async function showSection(sectionId) {
    ...
    switch (sectionId) { 
        ...
        case 'exchange': await loadAdminExchange(); break;
        case 'tasks': await loadAdminTasks(); break;
        case 'news': await loadAdminNews(); break;
    }
}
...
async function loadAdminNews() {
    ...
    document.getElementById('adminNewsList').innerHTML = news.map(n => `
        <div class="data-item">
            <span><strong>${n.title}</strong><br><small>${n.content}</small></span>
            <button onclick="deleteAdminNews(${n.id})" class="styled-button action-btn danger">Видалити</button>
        </div>`).join('');
    
    const r2 = await fetchWithAuth('/api/admin/exchange');
    const d = await r2.json();
    document.getElementById('newsAssetSelect').innerHTML = '<option value="">Жодного (Просто оголошення)</option>' + d.assets.map(a => `<option value="${a.id}">${a.name} (${a.symbol})</option>`).join('');
}
...
async function createAdminNews() {
    const data = {
        ...
        assetId: document.getElementById('newsAssetSelect').value ? parseInt(document.getElementById('newsAssetSelect').value) : null,
        effectPercent: document.getElementById('newsEffect').value ? parseFloat(document.getElementById('newsEffect').value) : null
    };
    if(!data.title || !data.content) return alert('Заповніть назву і текст');
    const r = await fetchWithAuth('/api/admin/news', { method: 'POST', body: JSON.stringify(data) });
    if(r.ok) { alert('Опубліковано!'); document.getElementById('newsTitle').value=''; document.getElementById('newsContent').value=''; showSection('news'); } else alert('Помилка');
}
...
async function loadAdminTasks() { const res = await fetchWithAuth('/api/admin/tasks'); const tasks = await res.json(); document.getElementById('adminTasksList').innerHTML = tasks.map(t => `<div class="data-item"><span><strong>${t.title}</strong> | Нагорода: ${t.reward} грн <br><small>${t.description}</small></span><button onclick="deleteAdminTask(${t.id})" class="styled-button action-btn danger">Видалити</button></div>`).join(''); await loadAdminUsers(); document.getElementById('rewardUserSelect').innerHTML = '<option value="">Оберіть користувача</option>' + adminData.users.map(u => `<option value="${u.id}">${u.full_name}</option>`).join(''); document.getElementById('rewardTaskSelect').innerHTML = '<option value="">Оберіть завдання</option>' + tasks.map(t => `<option value="${t.id}">${t.title}</option>`).join(''); }
async function createAdminTask() { const data = { title: document.getElementById('newTaskTitle').value, description: document.getElementById('newTaskDesc').value, reward: parseFloat(document.getElementById('newTaskReward').value) }; if(!data.title || !data.description || isNaN(data.reward)) return alert('Заповніть всі поля!'); const r = await fetchWithAuth('/api/admin/tasks', { method: 'POST', body: JSON.stringify(data) }); if(r.ok) { alert('Створено!'); document.getElementById('newTaskTitle').value=''; document.getElementById('newTaskDesc').value=''; document.getElementById('newTaskReward').value=''; showSection('tasks'); } else alert('Помилка'); }
async function deleteAdminTask(id) { if(!confirm('Видалити завдання?')) return; const r = await fetchWithAuth(`/api/admin/tasks/${id}`, { method: 'DELETE' }); if(r.ok) showSection('tasks'); else alert('Помилка'); }
async function rewardUserTask() { const data = { userId: parseInt(document.getElementById('rewardUserSelect').value), taskId: parseInt(document.getElementById('rewardTaskSelect').value) }; if(!data.userId || !data.taskId) return alert('Оберіть користувача та завдання!'); const r = await fetchWithAuth('/api/admin/tasks/reward', { method: 'POST', body: JSON.stringify(data) }); const result = await r.json(); if(r.ok) { alert('Винагороду успішно нараховано!'); } else alert(result.message || 'Помилка'); }
...
async function loadAdminExchange() { const res = await fetchWithAuth('/api/admin/exchange'); adminData.exchange = await res.json(); document.getElementById('adminExchangeAssetsList').innerHTML = adminData.exchange.assets.map(a => `<div class="data-item"><span><strong>${a.symbol}</strong> - ${a.name} (${a.type}) | Ціна: ${a.price.toFixed(2)} грн</span><div class="button-group" style="display:flex; align-items:center; gap:0.5rem;"><input type="number" id="adminAssetPrice_${a.id}" value="${a.price.toFixed(2)}" style="width:100px; padding:0.5rem;"><input type="number" step="0.001" id="adminAssetVol_${a.id}" value="${a.volatility}" style="width:80px; padding:0.5rem;" title="Волатильність"><button onclick="updateAssetPrice(${a.id})" class="styled-button action-btn warning" style="padding:0.5rem;">Змінити</button></div></div>`).join(''); const txsList = document.getElementById('adminExchangeTxsList'); if (adminData.exchange.transactions.length === 0) { txsList.innerHTML = '<p>Немає торгів.</p>'; } else { txsList.innerHTML = adminData.exchange.transactions.map(t => { const dt = new Date(t.timestamp.replace(' ', 'T') + 'Z').toLocaleString('uk-UA'); const typeText = t.type === 'buy' ? '🟢 Купив' : '🔴 Продав'; return `<div class="data-item" style="${t.status==='cancelled' ? 'opacity:0.5;' : ''}"><span>${dt} | <strong>${t.full_name}</strong> ${typeText} <strong>${t.amount} ${t.symbol}</strong> за ${t.total_cost.toFixed(2)} грн</span>${t.status === 'completed' ? `<button onclick="cancelExchangeTx(${t.id})" class="styled-button action-btn danger" style="padding:0.5rem;">Скасувати</button>` : 'Скасовано'}</div>`;}).join(''); } }
async function createExchangeAsset() { const data = { name: document.getElementById('exNewName').value, symbol: document.getElementById('exNewSymbol').value.toUpperCase(), type: document.getElementById('exNewType').value, price: parseFloat(document.getElementById('exNewPrice').value), volatility: parseFloat(document.getElementById('exNewVol').value) }; if(!data.name || !data.symbol || isNaN(data.price)) return alert('Заповніть форму'); const r = await fetchWithAuth('/api/admin/exchange/create-asset', { method: 'POST', body: JSON.stringify(data) }); if (r.ok) { alert('Створено'); showSection('exchange'); } else { const err = await r.json(); alert(err.message); } }
async function updateAssetPrice(id) { const price = parseFloat(document.getElementById(`adminAssetPrice_${id}`).value); const vol = parseFloat(document.getElementById(`adminAssetVol_${id}`).value); const r = await fetchWithAuth('/api/admin/exchange/update-asset', { method: 'POST', body: JSON.stringify({ assetId: id, price, volatility: vol }) }); if(r.ok) { alert('Оновлено'); showSection('exchange'); } else alert('Помилка'); }
async function cancelExchangeTx(id) { if(!confirm('Скасувати?')) return; const r = await fetchWithAuth('/api/admin/exchange/cancel', { method: 'POST', body: JSON.stringify({ transactionId: id }) }); if(r.ok) { alert('Скасовано'); showSection('exchange'); } else alert('Помилка'); }
