...
// АВТО-ЗАВДАННЯ (Список для фронтенду)
const AUTO_TASKS = [
...
function renderTasks() {
    ...
    AUTO_TASKS.forEach(task => {
        ...
        <div class="task-card ${isCompleted ? 'completed' : ''}">
            ...
            <div class="task-status ${isCompleted ? 'done' : 'todo'}">${isCompleted ? '✅ Отримано' : '⏳ Виконайте в додатку'}</div>
        </div>`;
    });

    // 2. Завдання від адміна
    if(appData.adminTasks && appData.adminTasks.length > 0) {
        ...
        appData.adminTasks.forEach(task => {
            ...
            <div class="task-card ${isCompleted ? 'completed' : ''}" style="border-left: 4px solid var(--primary-color);">
                ...
                <div class="task-status ${isCompleted ? 'done' : 'todo'}">${isCompleted ? '✅ Отримано' : '📝 Надішліть підтвердження адміну'}</div>
            </div>`;
        });
    }
    list.innerHTML = html;
...
// ... Існуючі функції (showSendMoney, showDepositModal, showExchangeModal і тд - БЕЗ ЗМІН) ...
...
function drawChart(assetId, assetName) { const ctx = document.getElementById('exchangeChart').getContext('2d'); const historyData = appData.exchange.history[assetId] || []; const labels = historyData.map(h => { const d = new Date(h.time.replace(' ', 'T') + 'Z'); return `${d.getHours()}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')}`; }); const dataPoints = historyData.map(h => h.price); if (chartInstance && currentChartAssetId === assetId) { chartInstance.data.labels = labels; chartInstance.data.datasets[0].data = dataPoints; chartInstance.update('none'); return; } if (chartInstance) chartInstance.destroy(); currentChartAssetId = assetId; chartInstance = new Chart(ctx, { type: 'line', data: { labels: labels, datasets: [{ label: assetName, data: dataPoints, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', borderWidth: 2, fill: true, tension: 0.3, pointRadius: 2, pointHoverRadius: 5 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { duration: 800, easing: 'easeOutQuart' }, scales: { x: { display: false }, y: { ticks: { color: '#9ca3af', font: { family: 'Inter' } }, grid: { color: 'rgba(255,255,255,0.05)' } } }, plugins: { legend: { display: false }, tooltip: { backgroundColor: 'rgba(31, 41, 55, 0.9)', titleColor: '#fff', bodyColor: '#10b981', bodyFont: { weight: 'bold' }, callbacks: { label: (ctx) => `${ctx.raw.toFixed(2)} грн` } } } } }); }
...
function updateActiveNavButton(screenName) { const mapping = { 'main': 1, 'shop': 2, 'exchange': 3, 'tasks': 4, 'personal': 5 }; document.querySelectorAll('.bottom-nav .nav-btn').forEach(b => b.classList.remove('active')); const btn = document.querySelector(`.bottom-nav .nav-btn:nth-child(${mapping[screenName] || 1})`); if(btn) btn.classList.add('active'); }
...
async function loadAdminTasks() {
    ...
    document.getElementById('adminTasksList').innerHTML = tasks.map(t => `
        <div class="data-item">
    ...
    await loadAdminUsers(); // для селекту
    document.getElementById('rewardUserSelect').innerHTML = '<option value="">Оберіть користувача</option>' + adminData.users.map(u => `<option value="${u.id}">${u.full_name}</option>`).join('');
    document.getElementById('rewardTaskSelect').innerHTML = '<option value="">Оберіть завдання</option>' + tasks.map(t => `<option value="${t.id}">${t.title}</option>`).join('');
}
...
async function createAdminTask() {
    ...
    if(!data.title || !data.description || isNaN(data.reward)) return alert('Заповніть всі поля!');
    const r = await fetchWithAuth('/api/admin/tasks', { method: 'POST', body: JSON.stringify(data) });
    if(r.ok) { alert('Створено!'); document.getElementById('newTaskTitle').value=''; document.getElementById('newTaskDesc').value=''; document.getElementById('newTaskReward').value=''; showSection('tasks'); } else alert('Помилка');
}
...
// ... Існуючі Адмін функції (Users, Teams, Shop, Exchange) ...
