...
async function login() {
    try {
        const response = await fetch(`${API_BASE_URL}/login`, {
            ...
            body: JSON.stringify({ login: document.getElementById('username').value.trim(), password: document.getElementById('password').value })
        });
    ...
    listDiv.innerHTML = Object.keys(grouped).map(dateKey => `
        ...
        ${grouped[dateKey].map(t => {
            ...
            <div class="transaction-item">
                ...
                <span class="transaction-amount ${isPositive ? 'positive' : 'negative'}">${isPositive ? '+' : ''}${parseFloat(t.amount).toFixed(2)}</span>
            </div>`
        }).join('')}
    `).join('');
}
...
function getTransactionIconByType(type) {
    ...
    if (type.includes('task_reward')) return '🎁';
...
function getTransactionTitle(t) {
    ...
    if (t.type === 'task_reward') return 'Виконано завдання';
    return 'Операція';
}
...
async function loadNews() {
    try {
        ...
        if (r.ok) {
            ...
            if (news.length === 0) {
                list.innerHTML = '<p class="no-transactions">Новин поки немає.</p>';
                return;
            }
            list.innerHTML = news.map(n => {
                const dt = new Date(n.timestamp.replace(' ', 'T') + 'Z').toLocaleString('uk-UA');
                return `
                <div class="task-card" style="border-left: 4px solid var(--primary-color);">
                    <div class="task-header"><h4 class="task-title">📰 ${n.title}</h4></div>
                    <p class="task-desc" style="margin-top:0.5rem; font-size:1rem; color:var(--text-primary);">${n.content}</p>
                    <div class="task-status" style="color:var(--text-disabled); justify-content:flex-start;">${dt}</div>
                </div>`;
            }).join('');
        }
...
async function showSection(sectionId) {
    ...
    try {
        ...
        console.error(`Помилка завантаження секції ${sectionId}:`, error);
    }
}
...
async function openEditUserModal(id) {
    ...
    const ts = document.getElementById('editTeam');
    ts.innerHTML = '<option value="">Без команди</option>' + adminData.teams.map(t => `<option value="${t.id}">${t.name}</option>`).join('');
    document.getElementById('editUserModalTitle').textContent = `Редагувати: ${u.full_name}`;
...
async function loadAdminTeamsAndUsers() {
    try {
        ...
        if (teamMembers) teamMembers.innerHTML = adminData.users.filter(x => !x.team_name).map(x => `<option value="${x.id}">${x.full_name}</option>`).join('');
        const bulkSelect = document.getElementById('bulkTeamSelect');
        if (bulkSelect) bulkSelect.innerHTML = '<option value="">Оберіть</option>' + adminData.teams.map(x => `<option value="${x.id}">${x.name}</option>`).join('');
        const teamList = document.getElementById('teamList');
        if (teamList) teamList.innerHTML = adminData.teams.map(x => `<div class="data-item"><span>${x.name}</span></div>`).join('');
    } catch (e) { console.error(e); }
}

async function createTeam() {
    const d = { name: document.getElementById('teamName').value, members: Array.from(document.getElementById('teamMembers').selectedOptions).map(o => o.value) };
    if (!d.name) return alert('Введіть назву');
    const r = await fetchWithAuth('/api/admin/teams', { method: 'POST', body: JSON.stringify(d) });
    if (r.ok) { alert('Створено!'); showSection('teams'); } else alert('Помилка');
}
...
async function addShopItem() {
    ...
    const m = currentEditShopItemId ? 'PUT' : 'POST';
    const r = await fetchWithAuth(url, { method: m, body: JSON.stringify(i) });
    if (r.ok) { alert('Збережено!'); clearShopForm(); showSection('shop'); } else alert('Помилка');
}

function editShopItem(id) {
    const i = adminData.shop.find(x => x.id === id);
    if (!i) return;
    currentEditShopItemId = id;
    document.getElementById('itemName').value = i.name;
    document.getElementById('itemPrice').value = i.price;
    document.getElementById('itemDiscountPrice').value = i.discount_price || '';
    ...
    document.getElementById('addShopItemBtn').textContent = 'Оновити';
...
function clearShopForm() {
    ...
    document.querySelector('#shop .form-group').querySelectorAll('input, textarea').forEach(e => e.value = '');
    document.getElementById('addShopItemBtn').textContent = 'Зберегти';
...
async function loadAdminDeposits() {
    try {
        ...
        if (!l) return;
        if (d.length === 0) { l.innerHTML = '<p style="padding:1rem;">Немає активних депозитів.</p>'; return; }
        l.innerHTML = d.map(x => {
...
async function adminCancelDeposit(id) {
    ...
    const r = await fetchWithAuth('/api/admin/deposits/cancel', { method: 'POST', body: JSON.stringify({ depositId: id }) });
    if (r.ok) { alert('Скасовано.'); showSection('deposits'); } else alert('Помилка.');
}
...
async function createExchangeAsset() {
    const data = {
        name: document.getElementById('exNewName').value,
        symbol: document.getElementById('exNewSymbol').value.toUpperCase(),
        type: document.getElementById('exNewType').value,
        price: parseFloat(document.getElementById('exNewPrice').value),
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (downloadDbBtn) {
        downloadDbBtn.addEventListener('click', () => {
            fetch('/api/admin/db/download', { headers: { 'Authorization': `Bearer ${token}` } })
                .then(res => { if (res.ok) return res.blob(); throw new Error('Помилка завантаження.'); })
                .then(blob => {
    ...
    if (uploadDbBtn && uploadDbInput) {
        ...
        uploadDbInput.addEventListener('change', (e) => {
            ...
            if (!file) return;
            if (!confirm('Увага! Це замінить базу. Продовжити?')) { uploadDbInput.value = ''; return; }
            const formData = new FormData();
            formData.append('file', file);
            fetch('/api/admin/db/upload', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData })
                .then(res => res.json())
                .then(data => { if (data.success) { alert('Оновлено!'); location.reload(); } else { alert('Помилка: ' + data.message); } })
