...
async def market_simulation_task():
    while True:
        ...
        try:
            if database.simulate_market_fluctuations(): await manager.broadcast({"type": "exchange_update_required"})
        ...
        while True:
            ...
            if data.get("type") == "register":
                ...
                if token:
                    ...
                    except jwt.PyJWTError: await websocket.close(); return
...
class AdminNewsCreate(BaseModel): title: str; content: str; assetId: Optional[int] = None; effectPercent: Optional[float] = None
@app.post("/api/admin/news")
async def admin_create_news(data: AdminNewsCreate, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); cursor = db.cursor()
    try:
        cursor.execute("BEGIN TRANSACTION")
        if data.assetId and data.effectPercent:
            asset = cursor.execute("SELECT price FROM exchange_assets WHERE id = ?", (data.assetId,)).fetchone()
            if asset:
                new_price = max(0.01, asset["price"] * (1 + (data.effectPercent / 100.0)))
                cursor.execute("UPDATE exchange_assets SET price = ? WHERE id = ?", (new_price, data.assetId))
                cursor.execute("INSERT INTO price_history (asset_id, price) VALUES (?, ?)", (data.assetId, new_price))
        cursor.execute("INSERT INTO news (title, content, asset_id, effect_percent) VALUES (?, ?, ?, ?)", (data.title, data.content, data.assetId, data.effectPercent))
        db.commit()
        await manager.broadcast({"type": "news_update_required"})
        if data.assetId: await manager.broadcast({"type": "exchange_update_required"})
...
async def admin_delete_news(news_id: int, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); db.execute("DELETE FROM news WHERE id = ?", (news_id,)); db.commit()
    await manager.broadcast({"type": "news_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
    return {"success": True}

class TaskSubmitData(BaseModel): taskId: int
@app.post("/api/tasks/submit")
async def submit_task(data: TaskSubmitData, user: dict = Depends(get_current_user)):
    db = database.get_db(); cursor = db.cursor()
    task_key = f"admin_{data.taskId}"
    if cursor.execute("SELECT id FROM completed_tasks WHERE user_id = ? AND task_key = ?", (user['id'], task_key)).fetchone(): return JSONResponse(status_code=400, content={"message": "Завдання вже виконано."})
    if cursor.execute("SELECT id FROM task_requests WHERE user_id = ? AND task_id = ? AND status = 'pending'", (user['id'], data.taskId)).fetchone(): return JSONResponse(status_code=400, content={"message": "Завдання вже на перевірці."})
    cursor.execute("INSERT INTO task_requests (user_id, task_id) VALUES (?, ?)", (user['id'], data.taskId)); db.commit()
    await manager.broadcast({"type": "full_update_required"}, [user["id"]]); await manager.broadcast({"type": "admin_panel_update_required"})
    return {"success": True, "message": "Заявку відправлено на перевірку!"}

@app.get("/api/admin/task-requests")
def get_task_requests(user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    reqs = database.get_db().execute("SELECT r.id, r.user_id, r.task_id, r.timestamp, u.full_name, t.title, t.reward FROM task_requests r JOIN users u ON r.user_id = u.id JOIN admin_tasks t ON r.task_id = t.id WHERE r.status = 'pending' ORDER BY r.timestamp ASC").fetchall()
    return [dict(r) for r in reqs]

class TaskProcessData(BaseModel): requestId: int; action: str
@app.post("/api/admin/task-requests/process")
async def process_task_request(data: TaskProcessData, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); cursor = db.cursor()
    try:
        cursor.execute("BEGIN TRANSACTION")
        req = cursor.execute("SELECT * FROM task_requests WHERE id = ? AND status = 'pending'", (data.requestId,)).fetchone()
        if not req: db.rollback(); return JSONResponse(status_code=400, content={"message": "Заявку не знайдено."})
        if data.action == 'approve':
            task = cursor.execute("SELECT * FROM admin_tasks WHERE id = ?", (req['task_id'],)).fetchone()
            cursor.execute("UPDATE task_requests SET status = 'approved' WHERE id = ?", (req['id'],))
            cursor.execute("INSERT INTO completed_tasks (user_id, task_key) VALUES (?, ?)", (req['user_id'], f"admin_{req['task_id']}"))
            cursor.execute("UPDATE users SET balance = balance + ? WHERE id = ?", (task['reward'], req['user_id']))
            cursor.execute("INSERT INTO transactions (user_id, type, amount, counterparty, comment) VALUES (?, 'task_reward', ?, ?, ?)", (req['user_id'], task['reward'], user['full_name'], f"Винагорода за: {task['title']}"))
        else: cursor.execute("DELETE FROM task_requests WHERE id = ?", (req['id'],))
        db.commit()
        await manager.broadcast({"type": "full_update_required"}, [req['user_id']]); await manager.broadcast({"type": "admin_panel_update_required"})
        return {"success": True}
    except Exception as e: db.rollback(); return JSONResponse(status_code=400, content={"message": str(e)})
...
async def admin_create_task(data: AdminTaskCreate, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); db.execute("INSERT INTO admin_tasks (title, description, reward) VALUES (?, ?, ?)", (data.title, data.description, data.reward)); db.commit()
    await manager.broadcast({"type": "full_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
    return {"success": True}

@app.get("/api/admin/tasks")
def admin_get_tasks(user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    return [dict(t) for t in database.get_db().execute("SELECT * FROM admin_tasks").fetchall()]

@app.put("/api/admin/tasks/{task_id}/toggle")
async def admin_toggle_task(task_id: int, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); cursor = db.cursor()
    task = cursor.execute("SELECT is_active FROM admin_tasks WHERE id = ?", (task_id,)).fetchone()
    cursor.execute("UPDATE admin_tasks SET is_active = ? WHERE id = ?", (0 if task['is_active'] == 1 else 1, task_id)); db.commit()
    await manager.broadcast({"type": "full_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
    return {"success": True}
...
async def admin_delete_task(task_id: int, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); db.execute("DELETE FROM admin_tasks WHERE id = ?", (task_id,)); db.commit()
    await manager.broadcast({"type": "full_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
    return {"success": True}
...
def admin_get_exchange(user: dict = Depends(get_current_user)):
    ...
    return {"transactions": [dict(t) for t in txs], "assets": [dict(a) for a in db.execute('SELECT * FROM exchange_assets ORDER BY type, name').fetchall()]}
...
async def admin_create_asset(data: AdminCreateAssetData, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("INSERT INTO price_history (asset_id, price) VALUES (?, ?)", (cursor.lastrowid, data.price)); db.commit()
        await manager.broadcast({"type": "exchange_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
        return {"success": True}
...
async def admin_cancel_exchange_tx(data: CancelExTxData, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if tx['type'] == 'buy':
            ...
            cursor.execute("UPDATE users SET balance = balance - ? WHERE id = ?", (tx['total_cost'], tx['user_id']))
            cursor.execute("UPDATE user_portfolio SET amount = amount + ? WHERE user_id = ? AND asset_id = ?", (tx['amount'], tx['user_id'], tx['asset_id']))
        cursor.execute("UPDATE exchange_transactions SET status = 'cancelled' WHERE id = ?", (tx['id'],)); db.commit()
        await manager.broadcast({"type": "full_update_required"}, [tx["user_id"]]); await manager.broadcast({"type": "exchange_update_required"}); await manager.broadcast({"type": "admin_panel_update_required"})
        return {"success": True}
    except Exception as e: db.rollback(); return JSONResponse(status_code=400, content={"message": str(e)})
...
async def create_deposit(data: DepositData, user: dict = Depends(get_current_user)):
    ...
    if res["success"]: await manager.broadcast({"type": "full_update_required"}, [user["id"]]); return {"success": True, "message": res["message"]}
...
async def admin_cancel_deposit(data: CancelDepositData, user: dict = Depends(get_current_user)):
    ...
    if res['success']: await manager.broadcast({"type": "admin_panel_update_required"}); await manager.broadcast({"type": "full_update_required"}, [res['user_id']]); return {"success": True}
    return JSONResponse(status_code=400, content={"message": res['message']})
...
async def purchase(data: PurchaseData, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        db.commit()
        await manager.broadcast({"type": "full_update_required"}, [user["id"]]); await manager.broadcast({"type": "shop_update_required"})
...
def admin_get_users(user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    return [dict(u) for u in database.get_db().execute("SELECT u.id, u.username, u.full_name, u.dob, u.balance, u.is_blocked, u.team_id, t.name as team_name FROM users u LEFT JOIN teams t ON u.team_id = t.id WHERE u.is_admin = 0 ORDER BY u.full_name").fetchall()]
...
async def admin_create_user(data: UserCreateData, user: dict = Depends(get_current_user)):
    ...
    try: 
        ...
        db.commit(); await manager.broadcast({"type": "admin_panel_update_required"})
...
async def admin_update_user(user_id: int, data: UserUpdateData, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db()
    sql = 'UPDATE users SET username = ?, full_name = ?, dob = ?, balance = ?, is_blocked = ?, team_id = ?'; params = [data.username, data.fullName, data.dob, data.balance, 1 if data.is_blocked else 0, data.team_id]
    ...
    await manager.broadcast({"type": "admin_panel_update_required"}); await manager.broadcast({"type": "full_update_required"}, [user_id])
    return {"success": True}

@app.delete("/api/admin/users/{user_id}")
async def admin_delete_user(user_id: int, user: dict = Depends(get_current_user)):
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); db.execute("DELETE FROM users WHERE id = ?", (user_id,)); db.commit()
    await manager.broadcast({"type": "admin_panel_update_required"}); return {"success": True}
...
async def admin_adjust_balance(data: AdjustBalanceData, current_user: dict = Depends(get_current_user)): 
    ...
    await manager.broadcast({"type": "admin_panel_update_required"}); await manager.broadcast({"type": "full_update_required"}, [data.userId])
    return {"success": True}
...
async def admin_create_team(data: TeamCreateData, user: dict = Depends(get_current_user)):
    ...
    db = database.get_db()
    try: 
        cursor = db.cursor(); cursor.execute('INSERT INTO teams (name) VALUES (?)', (data.name,)); team_id = cursor.lastrowid
        for m in data.members: cursor.execute('UPDATE users SET team_id = ? WHERE id = ?', (team_id, m))
        db.commit(); await manager.broadcast({"type": "admin_panel_update_required"}); return JSONResponse(status_code=201, content={"id": team_id})
...
async def admin_bulk_adjust(data: BulkAdjustData, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for u in users_in_team: 
            ...
            cursor.execute('INSERT INTO transactions (user_id, type, amount, counterparty, comment) VALUES (?, ?, ?, ?, ?)', (u["id"], 'admin_adjustment', final_amount, f"Масово ({current_user['full_name']})", data.comment)); updated_ids.append(u["id"])
        db.commit(); await manager.broadcast({"type": "admin_panel_update_required"}); await manager.broadcast({"type": "full_update_required"}, updated_ids)
...
async def admin_create_shop_item(data: ShopItemData, user: dict = Depends(get_current_user)):
    ...
    db.commit(); await manager.broadcast({"type": "shop_update_required"}); return JSONResponse(status_code=201, content={"id": cursor.lastrowid})
...
async def admin_update_shop_item(item_id: int, data: ShopItemData, user: dict = Depends(get_current_user)): 
    ...
    db.commit(); await manager.broadcast({"type": "shop_update_required"}); return {"success": True}

@app.delete("/api/admin/shop-items/{item_id}")
async def admin_delete_shop_item(item_id: int, user: dict = Depends(get_current_user)): 
    if not user.get("is_admin"): return JSONResponse(status_code=403, content={})
    db = database.get_db(); db.execute('DELETE FROM shop_items WHERE id = ?', (item_id,)); db.commit()
    await manager.broadcast({"type": "shop_update_required"}); return {"success": True}
...
async def upload_database(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    ...
    for t in ["full_update_required", "shop_update_required", "exchange_update_required", "admin_panel_update_required", "news_update_required"]: await manager.broadcast({"type": t})
